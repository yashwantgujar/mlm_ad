...
//           setLoading(true);
//           setError(null);
//           const response = await getProductByDealer(user._id);
...
//   CircularProgress,
// } from "@mui/material";
// import { getProductByDealer, getAllProductsSubDealer } from "../services/api";
...
//           if (user.role === "dealer") {
//             response = await getProductByDealer(user._id);
//           } else if (user.role === "subdealer") {
//             response = await getAllProductsSubDealer(user._id);
//           } else {
//             // For other roles, don't fetch bikes
//             setProducts([]);
//             setLoading(false);
//             return;
//           }
...
//           if (response && (response.bikes || response.products)) {
//             // Some APIs might use 'bikes' and others might use 'products' in response
//             const bikesData = response.bikes || response.products || [];
//             setProducts(bikesData);
//           } else {
//             setProducts([]);
//             console.warn("No bike data found in the response");
//           }
//         }
//       } catch (err) {
//         console.error("Error fetching bikes:", err);
...
//         <h2 className="text-center text-primary mb-4">Dealer Profile</h2>
//         <div className="row g-4">
//           {/* Bikes Section - Only show if user is dealer or subdealer */}
//           {(user.role === "dealer" || user.role === "subdealer") && (
//             <div className="col-md-12">
//               <div className="bg-light p-3 rounded">
//                 <h5 className="border-bottom pb-2 text-primary">
//                   {user.role === "dealer" ? "Assigned Bikes" : "Available Bikes"}
//                 </h5>
...
//                     <Typography variant="h5" color="text.secondary">
//                       {user.role === "dealer" 
//                         ? "No bikes assigned to you yet." 
//                         : "No bikes available for you yet."}
//                     </Typography>
//                   </Box>
//                 ) : (
//                   <Box sx={{ px: { xs: 2, sm: 3, md: 4 }, py: 5 }}>
...
//                       sx={{ mb: 4 }}
//                     >
//                       🚲 {user.role === "dealer" ? "Your Assigned" : "Available"} Electric Bikes
//                     </Typography>
...
const DealerTableView = () => {
 ...
 useEffect(() => {
    const fetchIncentives = async () => {
      ...
      try {
        const data = await  getAllProductsDealer();
       setbike(data);
       console.log(data);
          setProducts(data);
...
const handleViewDetails = (bikeId) => {
  navigate("/dealer-productdetails", { state: { bikeId } });
};
